...
int *cmp;
{
	...
	time1 = cnvtdate( d1, TRUE );	/* add TIME */
	time2 = cnvtdate( d2, TRUE );	/* add TIME */

	if ( time1 == 0 || time2 == 0 )
		return( FALSE );

	*cmp = ( time1 < time2 ? -1 : time1 > time2 ? 1 : 0 );
...
int *cmp;
{
	...
	time1 = cnvtdate( d1, FALSE );	/* do not add TIME */
	time2 = cnvtdate( d2, FALSE );	/* do not add TIME */
